...
Use GPT-5.6 Luna for lightweight triage, task distribution, repository reconnaissance, and bounded routine work. Use the Fable model only for complex architecture or planning when that depth is needed. Use Claude Code Sonnet for implementation and Claude Code Opus for code review, security review, and high-risk verification. Use Grok for a targeted second opinion or research pass after Claude when it adds value. Keep OpenCode Go for economical bulk or fallback work. Minimize GPT usage outside Luna triage and use a higher-cost GPT model only when an external provider is unavailable or the task explicitly requires it.
