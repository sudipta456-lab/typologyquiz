...
## September 20 release: Humanizer-reviewed Mental Age report

Search Console showed early demand for the Mental Age page without clicks, so its public result language received a narrow Humanizer review. The revised copy keeps the quiz playful and explicitly non-clinical while replacing a few generic or abstract lines with more specific details. Questions, score calculations, evidence label, and disclaimer did not change.

The report is versioned as `mental-age@2026-09-20.1`. It carries forward the existing instrument and scoring version (`2026-09-11.1`) and adds only a new immutable report version, so old result links retain the text visitors originally received. The release command now accepts explicitly named test slugs for a full release and has a guarded `--report-only` mode for wording changes. That mode requires at least one named test, rejects duplicate or unknown slugs, and refuses to preserve a scoring version if the scoring fingerprint changed.

Claude Sonnet performed the initial Humanizer audit. Claude Opus reviewed the copy and release mechanism twice; its final review approved the implementation after the version and argument guards were added. Focused typecheck, assessment lint, and all 16 assessment-quality tests passed. A clean worktree then passed the complete 76-check gate, built 1,248 static routes, and passed the 536-route sitemap/discovery check. Cloudflare version `ee3c1271-dda2-4d82-8585-8bdd3c0d89a7` deployed to both custom domains. Both live Mental Age route requests returned HTTP 200, and the deployed build contained the revised result copy.
