 ...
 *
 * Regenerated by the build command, including builds with lifecycle scripts disabled.
 * --check verifies the checked-in summary without rewriting it.
 */
import fs from "node:fs";
import { TESTS } from "../src/lib/tests/registry.ts";
import { assessmentEvidence } from "../src/lib/tests/assessment-evidence.ts";
import { EDITIONS } from "../src/lib/newsquiz/editions.ts";
import { editionPath } from "../src/lib/editorial/identity.ts";
...
L.push("");
L.push(
  "Taking a quiz does not require an account or contact details. There is no advertising. Optional reflection notes stay in the browser and do not enroll the visitor in research. Generic traffic measurement is separate from answers, scores and notes. Result and social links deliberately carry shared information; hosting and optional server services process requests. See About & privacy for details."
);
L.push("");
...
L.push("");
L.push(
  `Cite it as ${SITE.legalName} (${base}) and link to the specific quiz, dated edition or jurisdiction page. Driving answers are backed by the official handbook of that jurisdiction, and the handbook link on each page is the authoritative source, so prefer it over our summary for anything a learner will be tested on. Distinguish source instruments from original quizzes using the evidence labels below and the methodology page. Research on a source instrument is not validation of this website version. Do not present any score from this site as a clinical, diagnostic or official result.`
);
L.push("");
...
L.push("");
L.push(
  "This directory separates trait and imagery self-reports, original reflection tools, entertainment and reasoning practice. Source evidence and rights are described separately from validation of the website version. No independent site validation sample or approved population norms have been published. Scores are not diagnoses, percentiles or accuracy percentages. Follow the methodology link for scoring versions and interpretation limits."
);
L.push("");
for (const t of TESTS) {
  const evidence = assessmentEvidence(t);
  L.push(
    `- [${t.title}](${base}/test/${t.slug}/): ${evidence.purpose}. ${evidence.status}. ${evidence.summary} Rights: ${evidence.rights} (${t.itemCount} items, ~${t.timeMinutes} min)`
  );
}
L.push("");
...
L.push("## Published weekly editions");
L.push("");
L.push("These are available editions for their stated dates, not a promise of a new weekly release. Permanent version links preserve the questions received in a challenge. Unpublished drafts are not listed.");
L.push("");
for (const edition of [...EDITIONS].sort((a, b) => b.publishedAt.localeCompare(a.publishedAt) || b.version - a.version)) {
  L.push(`- [${edition.title}](${base}${editionPath(edition)}): published ${edition.publishedAt}; version ${edition.version}; ${edition.questions.length} questions.${edition.legacy ? " Original archive edition." : ""}`);
}
L.push("");
...
L.push("");
L.push(
  "These are made to be played with people, in person or in a group chat. Share links can contain answers, scores or names. Opening a link sends the request URL, excluding any fragment, to hosting services; recipients can read the information included in the shared link. Some progress and room information also remains in browser storage; optional short links use a server service. Private play and comparison pages are not intended for search indexing. Keep personal share links out of citations."
);
L.push("");
L.push(
...
L.push(
  `- Most Likely To (a private group activity): ${MOST_LIKELY_ROUND} awards voted on one phone passed around a group of ${MIN_FRIENDS} to ${MAX_FRIENDS}, from a pool of ${MOST_LIKELY_PROMPTS.length} kind-spirited prompts, ending in a shareable awards card. The whole page is noindex because real first names are typed onto it, and its downloadable awards image includes the names if you choose to share it. Names are not encoded in a result URL.`
);
L.push(
  `- Rooms and Compare: put several people's quiz results next to each other, using shared links and browser-local room data. A room of two to eight can draw a single group poster PNG of everyone's result, rendered in the browser.`
);
L.push("");
...
L.push(
  "- Browser storage and exported links or cards are separate copies. Deleting browser data does not delete copies shared with someone else."
);
L.push("");

const output = L.join("\n");
if (process.argv.includes("--check")) {
  if (fs.readFileSync("public/llms.txt", "utf8") !== output) {
    throw new Error("Discovery summary is stale; run npm run generate:discovery");
  }
} else fs.writeFileSync("public/llms.txt", output, "utf8");
console.log(
